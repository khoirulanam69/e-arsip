...
class Arsip extends CI_Controller
{
    ...
    public function processArsipBaru()
    {
        ...
        if (!$fail) {
            ...
            redirect(base_url("arsip/manajemen/".$idArsip."?status=1&message=Penambahan arsip berhasil."));
        }
    }
    ...
    public function manajemen($idArsip = null)
    {
        ...
        $data['sidemenu'] = array(
            array(
                ...
                'icon' => 'folder',
                'url' => base_url("arsip/manajemen/$idArsip"),
                'active' => true
            ),
        );
    ...
    public function processManajemen()
    {
        ...
        if (!$fail) {
            ...
            redirect(base_url("arsip/manajemen/".$idArsip."?status=1&message=Update arsip berhasil"));
        }
    }

    public function hapusArsip($id = null)
    {
        if ($id === null) {
            redirect(base_url());
        }
        ...
        $affRows = $this->m_model->deleteArsip($where);
        
        if ($affRows <= 0) {
            redirect("arsip/bukuagenda?status=-1&message=Hapus data gagal.");
        } else {
            redirect("arsip/bukuagenda?status=1&message=Hapus data berhasil.");
        }
    ...
    public function processPinjam()
    {
        ...
        if ($affRows <= 0) {
            ...
            redirect(base_url("arsip/manajemen/$idArsip?status=1&message=Proses pinjam arsip berhasil."));
        }
    }
    ...
    public function processKembali()
    {
        ...
        if ($affRows <= 0) {
            redirect(base_url("arsip/manajemen/$idArsip?status=-1&message=Proses pengembalian arsip gagal!"));
        }
        else {
            redirect(base_url("arsip/manajemen/$idArsip?status=1&message=Proses pengembalian arsip berhasil."));
        }
    }
    ...
    public function processUser()
    {
        ...
        $affRows = $this->m_model->updateUser($updateData, $idUser);

        if ($affRows < 0) {
            redirect(base_url("arsip/usersettings?status=-1&message=Update informasi akun gagal!"));
        }
        else {
            redirect(base_url("arsip/usersettings?status=1&message=Update informasi akun berhasil."));
        }
    }
